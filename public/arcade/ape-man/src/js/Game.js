...
import ApePacman from './ApePacman.js';
...
class JsPacman extends Game {
    constructor(options = {}) {
        ...
        this.elements = {
            ...
            paused : this.$('.paused'),
            gameOverScreen : this.$('.game-over-screen'),
        ...
        if (this.elements.mainMenuButton) {
            this.elements.mainMenuButton.addEventListener('click', () => {
                // Simply reload the page for a complete reset
        ...
        if (this.elements.playAgainButton) {
            this.elements.playAgainButton.addEventListener('click', () => {
                if (this.elements.gameOverScreen) {
                    hide(this.elements.gameOverScreen);
                }
    ...
    makeLevel() {
        ...
        // Pacman (use ApePacman for ape character support).
        this.pacman = new ApePacman({
            preturn : true,
            x : 452,
            y : 848,
    ...
    mainLoop() {
        ...
        if (!this._pauseFrames) {
            ...
            if (this._gameOver) {
                // Don't hide the game over screen when game is over
                return;
            }
    ...
    _getInputDirection() {
        ...
        // Check multiple mobile input sources
        const mobileDirection = this._mobileInputDirection || 
                               (window.mobileControlsGame && window.mobileControlsGame._mobileInputDirection) ||
                               window._mobileInputDirection;
        ...
        // If keyboard input, clear other inputs
        if (direction) {
            this._lastSwipe = null;
            this._mobileInputDirection = null;
            window._mobileInputDirection = null;
        }
        // Check for mobile button input (with fallbacks)
    ...
    _onChangeLives(model, lives) {
        if (lives === 0) {
            ...
            console.log('Game over! Final score:', this.model.score);
            
            // Hide all game elements
            if (this.elements.start) hide(this.elements.start);
            if (this.elements.startP1) hide(this.elements.startP1);
            if (this.elements.startReady) hide(this.elements.startReady);
            if (this.elements.gameOver) hide(this.elements.gameOver);
            
            // Hide game characters
            this.hideGhosts();
            if (this.pacman) this.pacman.hide();
            
            // Show game over screen
            if (this.elements.gameOverScreen) {
                show(this.elements.gameOverScreen);
                // Update score in game over screen
                const scoreElement = this.elements.gameOverScreen.querySelector('p');
                if (scoreElement) {
                    scoreElement.textContent = `Score: ${this.model.score}`;
                }
            }
            
            // Note: Score saving is now handled by the unified achievement system in the HTML file
            // This prevents conflicts between different database implementations
