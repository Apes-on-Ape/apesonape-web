...
class Animation {
    constructor(options) {
        ...
        // Resolve sprite paths relative to the deployed bundle (`.../dist/img/`), set in index.html / index.js
        if (this.imageURL && !this.imageURL.startsWith('http')) {
            const cleanPath = this.imageURL.replace(/undefined/g, '').replace(/^\/+/, '');
            const base =
                (typeof window !== 'undefined' && window.JSPACMAN_IMG_PATH) ||
                (typeof window !== 'undefined' ? `${new URL('./dist/img/', window.location.href).href}` : '');
            const prefix = String(base).replace(/\/?$/, '/');
            this.imageURL = `${prefix}${cleanPath}`;
        }
    }
