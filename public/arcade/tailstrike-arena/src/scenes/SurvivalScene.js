...
import { buildCharacterSpells } from '../data/characterSpellDefinitions';
import {
  registerSurvivalSpellOverlaps,
  spawnEnemySpellTowardPlayer,
  spawnPlayerSpell,
} from '../survival/survivalSpellEffects';
...
export default class SurvivalScene extends Phaser.Scene {
  ...
  spawnNextEnemyOrFinish() {
    ...
    spr.setData('shootDelay', this.enemyShootBaseDelay - Math.min(800, this.wave * 40) + Phaser.Math.Between(-200, 200));
  ...
  updateEnemies(time) {
    this.enemies.getChildren().forEach((e) => {
      ...
      const speed = e.getData('isBoss') ? 55 : 70 + Math.min(this.wave * 2, 60);
  ...
  enemyShoot(enemy) {
    ...
    const base = enemy.getData('isBoss') ? 1.4 : 1;
    const dmg = Math.max(6, Math.floor(spell.damage * base * (1 + this.wave * 0.04)));
  ...
  onWaveCleared() {
    ...
    this.time.delayedCall(1400, () => {
      this.waveClearing = false;
      this.wave += 1;
      this.enemyShootBaseDelay = Math.max(700, 1800 - this.wave * 35);
      this.startWave();
    });
    this.updateHud();
  }
