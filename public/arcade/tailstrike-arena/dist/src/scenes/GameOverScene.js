export default class GameOverScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (this.isSurvival) {
      this.saveStatusText = this.add.text(width / 2, height / 2 + 28, 'Saving run...', {
        fontSize: '16px',
        fontFamily: 'Arial, sans-serif',
        fill: '#8be9fd',
        fontStyle: 'bold',
      }).setOrigin(0.5);
      this.saveTailstrikeScore().catch(() => {});
    }
  ...
  async saveTailstrikeScore() {
    if (this.scoreSaved) return;
    this.scoreSaved = true;
    const liveScore =
      typeof window !== 'undefined' ? Number(window.__tailstrikeLastSurvivalScore || 0) : 0;
    const score = Math.max(0, Number(this.survivalScore || 0), liveScore);
    if (!score || typeof window === 'undefined' || !window.GameScoreUtils) {
      if (this.saveStatusText) this.saveStatusText.setText('Run complete');
      return;
    }
    try {
      const ok = await window.GameScoreUtils.saveGameScore('tailstrike_arena', score, {
        success: this.result === 'victory',
      });
      if (this.saveStatusText) this.saveStatusText.setText(ok ? 'Run saved' : 'Save failed');
    } catch (_) {
      if (this.saveStatusText) this.saveStatusText.setText('Save failed');
    }
  }
} 